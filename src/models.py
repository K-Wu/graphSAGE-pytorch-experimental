...
class GraphSage(nn.Module):
            ...
            if self.unique_after_sample:
                ...
                cur_hidden_embs = sage_layer(
                    self_feats=pre_hidden_embs[
                        :, -1
                    ],  # TODO: KWU: check if this indexing scheme is correct
    ...
    def _get_unique_neighs_list(self, nodes, num_sample=10):
        ...
        if not num_sample is None:
            ...
            _sample = (
                lambda x, k: np.random.choice(x, k, replace=True)
                if len(x) < k
                else np.random.choice(x, k, replace=False)
            )
            samp_neighs = [
                _set(_sample(to_neigh, num_sample)) for to_neigh in to_neighs
            ]
        else:
    ...
    def _get_neighs_list_without_unique(self, nodes, num_sample=10):
        ...
        samp_neighs = [list(_sample(to_neigh, num_sample)) for to_neigh in to_neighs]
    ...
    # TODO: KWU: separate this implementation to 1) batched implementation for num_sample is not None and 2) the current naive-for loop for num_sample is None
    # this is the original aggregate function in the original repo. It uses SpMM for to aggregate neighbors deduplicated (unique) after sampling.
    def aggregate_for_deduplicated(
        self, nodes, pre_hidden_embs, pre_neighs, num_sample=10
    ):
        ...
        if self.agg_func == "MEAN":
            ...
            if num_sample is None:
                # It could be the case that each node has different number of neighbors. In this case, we need to use for loop to aggregate neighbors.
                row_indices = [
                    i
                ...
                # each node has the same number of neighbors. We can use pytorch reduction function here.
                # first collect all neighbors into a single tensor of shape (num_nodes, num_sample, embed_dim)
                # then we can use the reduction function to aggregate neighbors
                # do not use mask directly because they are costly
                # use index
                neighbors_per_node = embed_matrix[
                    column_indices
                ]  # .view(len(samp_neighs), num_sample, embed_matrix.size(1))
                aggregate_feats = torch.max(neighbors_per_node, 1)[0].squeeze()
        else:
            raise NotImplementedError
